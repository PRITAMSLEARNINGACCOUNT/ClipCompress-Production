      ...
      try {
        ...
        if (File?.type.includes("video")) {
          ...
          const transformedUrl = `https://res.cloudinary.com/${
            ...
            Encoding.includes("H.264") ? "vc_h264" : "vc_h265"
          }/${data.public_id}.${Format.toLowerCase()}`;
          const Result = await fetch(transformedUrl as string);
          const BlobData = await Result.blob();
          const Result2 = await Upload_Video(
            File.size,
            data.public_id,
            BlobData.size,
            transformedUrl
          ...
          setVideoURL(Result2.URL);
          setCompressionRate(
            Math.round(
              (1 -
                Number(Result2.compressedSize) / Number(Result2.originalSize)) *
                100
            )
          );
          setOriginalSize(filesize(Result2.originalSize));
          setCompressedSize(filesize(Result2.compressedSize));
          return;
        } else {
